...
def _ensureHaveAccountPlayerProfile():
    ...
    if ('Player Profiles' not in config
        and '__account__' not in config['Player Profiles']):
...
def getPlayerProfileIcon(profileName):
  ...
  #  if profileName == '__modpack__': isGlobal = True
    if isGlobal:
...
def getLastPlayerNameFromInputDevice(device):
    ...
    if ('Default Player Profiles' in bsConfig
            or keyName in bsConfig['Default Player Profiles']):
    ...
    if profileName == '__pass__': profileName = ''
    if '_i:' in profileName:
        icon = random.choice([u'\ue043', u'\ue042', u'\ue041'])
        profileNameSave = profileName.split('_i:')[1:]
        profileName=''
        for i in profileNameSave: 
            profileName += i
        profileName = icon + profileName
    return profileName
...
def printLiveObjectWarnings(when, ignoreSession=None, ignoreActivity=None):
    ...
    for actor in actors:
        ...
        if type(actor) is bs.Actor:
            try:
                if actor.node.exists() and actor.isAlive(): 
                    actor.__del__()
            except Exception as e:
                print '   - exception deleting actor node:', e
...
class Background(bsGame.Actor):
    ...
    def __init__(self, fadeTime=500, startFaded=False, showLogo=False):
        ...
        with bs.Context(session):
            ...
            if showLogo:
                logoTexture = bs.getTexture('circle')
                logoModel = bs.getModel('achievementOutline')
                logoModelTransparent = bs.getModel('shield')
                ...
                if not bs.getEnvironment()['vrMode']:
                    ...
                    for attr in ['input0', 'input1']:
                        animate(self.c, attr, {0:0.193, 2000:0.2, 4000:0.193},
                                loop=True)
                    ...
                    animate(c, "input0", {0:1, 3000:-1}, loop=True)
                    animate(c, "input1", {0:-0.56, 3000:-0.55, 6000:-0.56}, loop=True)
                    animate(self.logo, "rotate", {0:0, 2000:360}, loop=True)
