...
gSettingsEnabled = hasattr(bs, 'get_setting')
...
class FatalityAnimateMessage(object):
    def __init__(self, damage=1000):
        self.damage = damage
        if hasattr(bs, "get_tint") and hasattr(bs, "get_motion"):
            self.tint, self.motion = bs.get_tint(), bs.get_motion()
            bs.shakeCamera(self.damage * 0.0005)
            def anim():
                bsUtils.animateArray(bs.getSharedObject("globals"), "tint", 3, {0: self.tint, 31: (0, 0, 0.2), 461: self.tint})
                bsUtils.animate(bs.getSharedObject("globals"), "slowMotion", {0: self.motion, 31: not self.motion, 300: self.motion})
            anim()
            self.check_timer = bs.Timer(461, bs.Call(self.check))
    def check(self):
        self.tint, self.motion = bs.get_tint(), bs.get_motion()
        if self.tint != bs.getSharedObject("globals").tint: bs.getSharedObject("globals").tint = self.tint
        if self.motion != bs.getSharedObject("globals").slowMotion: bs.getSharedObject("globals").slowMotion = self.motion
...
class Actor(object):
    ...
    self.flag = bs.Flag(position=(0,20,0))
...
class Session(object):
    ...
    def end(self):
        ...
        #    if isinstance(obj, bs.Actor) and "node" in dir(obj) and obj.exists: obj.node.delete(), obj.handleMessage(bs.DieMessage(immediate=True))
...
class Activity(object):
    ...
    def __init__(self, settings={}):
        ...
        self._useLobby = True
        self._lobby = None
        if hasattr(bs, 'set_motion'): bs.set_motion(None)
        if hasattr(bs, 'set_tint'): bs.set_tint(None)
    ...
    def onBegin(self):
        ...
        self.timers = [bs.Timer(1000, bs.Call(bsInternal._setPartyIconAlwaysVisible, True), repeat=True)]
        try: import chatCmd
        except ImportError: print("cann\'t find chatCmd-script")
        else: self.timers.append(bs.Timer(1000, bs.Call(chatCmd.update), repeat=True))
        if hasattr(bs, "check_settings_file"): bs.check_settings_file()
...
class GameActivity(Activity):
    ...
    def onTransitionIn(self, music=None):
        ...
        mapMotion = True if 'Epic Mode' in self.settings and self.settings['Epic Mode'] else False
        mapName = self._map.getName()
    ...
    def showScoreBoardInfo(self):
        ...
        y -= 16
        if not gSettingsEnabled or (gSettingsEnabled and bs.get_setting("show_game_name", True)):
            self._gameScoreBoardNameText = bs.NodeActor(
                bs.newNode("text",attrs={'text': sbName, 
                    'maxWidth': 300, 'position': (15, y)
                    if isinstance(self.getSession(),bs.FreeForAllSession) 
                    else(15, y),'hAttach': "left", 'vrDepth': 10,
                    'vAttach': "top",'vAlign': 'bottom', 
            ...
            self._gameScoreBoardDescriptionText = bs.NodeActor(
                bs.newNode("text",attrs={'text': sbDesc, 'maxWidth': 480,
                    'position': (17, -44 + 10)
                    if isinstance(self.getSession(),bs.FreeForAllSession) 
                    else(17, -44 + 10),'scale': 0.7, 'hAttach': "left", 
                    'vAttach': "top",'vAlign': 'top', 'shadow': 1.0 if vr else 0.7,
            ...
            try:
                bsUtils.animate(self._gameScoreBoardDescriptionText.node,'opacity', {0: 1, 1000: 0})
                bsUtils.animate(self._gameScoreBoardNameText.node,'opacity', {0: 1, 1000: 0})
            except: pass
            try:
                self._gameScoreBoardNameText = None
                self._gameScoreBoardDescriptionText = None
            except: pass
    ...
    def _standardDropPowerup(self, index, expire=True):
        import bsPowerup
        powerup = bs.Powerup.getFactory().getRandomPowerupType()
    ...
    def cameraFlash(self, duration=999, color=(0.2,0.2,0.8)):
        """
