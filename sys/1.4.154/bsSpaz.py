...
class Spaz(bs.Actor):
    ...
    def handleMessage(self, msg):
        ...
        if isinstance(msg, bs.PickedUpMessage):
        ...
        elif isinstance(msg, bs.DirtBombOutMessage):
            if not self.node.exists(): return
            self.slow = False
        ...
        elif isinstance(msg, bs.FireworkBombHitMessage):
            time = 4000
            if not self.node.exists(): return
            self.node.punchMaterials, self.node.rollerMaterials, self.node.extrasMaterials = [], [], []
            ...
            if msg.immediate:
                self.node.delete()
                if hasattr(self, "effects"): self.effects.delete()
                ...
                if hasattr(self, "effects"): bs.gameTimer(2000, self.effects.delete)
    ...
    def _getBombTypeTex(self):
        ...
        elif self.bombType == 'poison': return bs.getTexture('eggTex2')
...
class EffectHurt(object):
    def __init__(self, owner=None):
        ...
        bs.write_log()
        self._hurt = {}
        if gSettingsEnabled and not bs.get_setting("timer_the_disappearance_of_the_effect", False): return
    ...
    def add(self, text="test", time=5000, color=(1,1,1)):
        ...
        if hasattr(self, "_hurt"):
            ...
            if "timer" in self._hurt and self._hurt["timer"] is not None and self._hurt["timer"].exists():
                self._hurt["timer"].delete()
